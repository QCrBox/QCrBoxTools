...
from qcrboxtools.cif.file_converter.shelxl import cif2afix, afix2cif
...
@pytest.mark.parametrize("afix_line, expected", [
    ('AFIX 6', (0, 6)),
    ('AFIX 23', (2, 3)),
    ('AFIX 137', (13, 7))
])
...
@pytest.fixture(name='minimal_cif_block', params=[
...
def test_afix2cif_update_tables(minimal_cif_block):
    ...
    for column in changed_columns:
        assert all(float(new) != float(old) for new, old in zip(result[column], original[column])), f"{column} should be different"
