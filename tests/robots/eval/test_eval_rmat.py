...
expected_rmat = np.array([[0.1600435, 0.0135282, 0.0149924],
                            [-0.0090313, -0.0167682, 0.1045150],
                            [0.0466450, -0.0496632, -0.0312043]])

expected_tmat = np.array([[1.0000000, 0.0000000, 0.0000000],
                            [0.0000000, 1.0000000, 0.0000000],
                            [0.0000000, 0.0000000, 1.0000000]])
...
def test_to_rmat_file(tmp_path):
    ...
    for line, expected_line in zip(content, expected_content):
        assert line == expected_line, f"Line in file does not match expected line: {line.strip()} != {expected_line.strip()}"